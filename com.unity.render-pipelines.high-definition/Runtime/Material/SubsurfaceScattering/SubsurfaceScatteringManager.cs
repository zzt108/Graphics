            ...
            RTHandle diffuseBufferRT, RTHandle depthStencilBufferRT, RTHandle depthTextureRT)
        {
            ...
            if (hdCamera.frameSettings.IsEnabled(FrameSettingsField.RayTracing) && settings.rayTracing.value && GetRayTracingState())
            {
                using (new ProfilingScope(cmd, ProfilingSampler.Get(HDProfileId.SubsurfaceScattering)))
                {
                    ...
                    RayTracingShader subSurfaceShader = m_defaultSettings.renderPipelineRayTracingResources.subSurfaceRayTracing;
                    ComputeShader deferredRayTracing = m_defaultSettings.renderPipelineRayTracingResources.deferredRaytracingCS;
                    ComputeShader rayTracingSubSurfaceCS = m_defaultSettings.renderPipelineRayTracingResources.subSurfaceRayTracingCS;
